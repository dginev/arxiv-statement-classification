...
def load_data(path='data/sandbox_ams_1k.npz', num_words=200_000, skip_top=0,  # _1m
              maxlen=None, test_split=0.2, seed=521, shuffle=True,
              start_char=1, index_from=2, setup_labels=False, full_data=False, max_per_class=5_000,
    ...
    print('-- loading data...')
    xs, labels = [], []
    # full_data is a special flag, but it's also a hack, difference being between
    # full_data = True - expect HDF5 file
    # full_data = False - expect npz file
    # also, full_data disables some of the RAM-intensive paths, since I kept running out of memory.
    #
    # For now, to be safe, just use "full_data=False" (the default) and make sure you're using a reasonable
    # size for the .npz file to not run out of RAM. The code would need a major refactor if we want to work with data
    # which doesn't fit RAM at once, it gets a lot more hairy.
    # I cheated by virtue of having 64 GB of RAM on the machine where this was being run.
    if full_data:
        max_per_class = None
    ...
    if max_per_class != None:
        ...
        print("-- reducing data to max %d per class..." % max_per_class)
        selection_counter = {}
        xs_reduced = []
        labels_reduced = []
    ...
    if not(full_data):
        ...
        gc.collect()
        # NOTE: Assume the oov word is preprocessed outside of loading, save time.
        #       in particular, ams_tar_to_hdf5.py drops unknown words entirely
        # print("- oov word")
        if maxlen:
